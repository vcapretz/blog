...
One of the ways to make and app feel more polished is by introducing animations and transitions that lead to a better visual feedback when users are interacting with your app.
...
React Native has its own animation APIs and they work great for a lot of cases. The issue is that for more complex scenarios it will not support running those animations in the native thread, which them can cause performance issues and unresponsiveness since it then competes resources with JavaScript.
...
One of the goals being to make it easier for developers to use and understand, so I've been using the version 2 for some time and I think you should too!
...
> We will now use numbers instead of booleans so we can interpolate them and use directly into the styles as well.
